...
export const APRoutes = ({ toastRef }) => {
  const ROUTES = [
    { name: '/', value: (index) => (<Home key={index} updatePageTitle={updatePageTitle} toastRef={toastRef} />) },
    { name: '/auth', value: (index) => (<Auth key={index} updatePageTitle={updatePageTitle} toastRef={toastRef} />) },
    { name: '/home', value: (index) => <Home key={index} updatePageTitle={updatePageTitle} toastRef={toastRef} /> },
    { name: '/home/:listId', value: (index) => <PropertyView key={index} updatePageTitle={updatePageTitle} toastRef={toastRef} /> },
  ...
  return (
    <Routes>
      {ROUTES.map((route, index) => (
        <Route path={route.name} element={isLoggedIn ? route.value(index) : <Auth key={index} updatePageTitle={updatePageTitle} toastRef={toastRef} />} key={route.name} />
      ))}
